    ...
    try {
       ...
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'role'`);
        const colCount = Array.isArray(colRows) && colRows[0] && colRows[0].cnt
            ...
            : 0;
        if (colCount === 0) {
            await db_1.default.query("ALTER TABLE users ADD COLUMN role ENUM('student','professor') NOT NULL DEFAULT 'student' AFTER password_hash");
...
async function ensureSubmissionsTable() {
    try {
        const [tableRows] = await db_1.default.query(`SELECT COUNT(*) AS cnt
       FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'submissions'`);
        const tableCount = Array.isArray(tableRows) && tableRows[0] && tableRows[0].cnt
            ? Number(tableRows[0].cnt)
            : 0;
        if (tableCount === 0) {
...
async function ensureSubmissionColumns() {
    try {
        ...
        for (const col of expectedColumns) {
            try {
                const [rows] = await db_1.default.query(`SELECT COUNT(*) AS cnt FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'submissions' AND COLUMN_NAME = ?`, [col.name]);
...
async function seedInitialAdmin() {
    ...
    try {
        ...
        const [rows] = await db_1.default.query('SELECT id FROM users WHERE email = ? AND role = ? LIMIT 1', [admin.email, 'professor']);
        const exists = Array.isArray(rows) && rows.length > 0;
        if (!exists) {
            const passwordHash = await bcrypt_1.default.hash(admin.password, 10);
            await db_1.default.query('INSERT INTO users (name, email, matricula, password_hash, role) VALUES (?, ?, ?, ?, ?)', [admin.name, admin.email, admin.registry, passwordHash, 'professor']);
